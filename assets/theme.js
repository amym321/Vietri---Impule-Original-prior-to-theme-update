  ...
  $(function() {
    ...
    $('body').on('submit','form.cart',function(evt) {
      if($(document.activeElement).attr('name') == 'update') {
        evt.preventDefault();
        var promo_data = {
          'promo_spend_get_1_amount': "",
          'promo_spend_get_1_variant': "",
          'promo_spend_get_1_price': "",
          'promo_spend_get_1_from': "",
          'promo_spend_get_1_to': "",
        };
        var today = new Date();
        var promo1 = {
          'from' : new Date(promo_data.promo_spend_get_1_from),
          'to': new Date(promo_data.promo_spend_get_1_to)
        };
        var min_purchase = parseInt(promo_data.promo_spend_get_1_amount); // + parseInt(promo_data.promo_spend_get_1_price);
        //is there an active campaign
        theme.cart.getCart()
          .then(function (cart) {
            if(today >= promo1.from && today <= promo1.to) {
              $promo_variant = cart.items.find(function (value) {
                return value.id == promo_data.promo_spend_get_1_variant;
              });
              if (cart.original_total_price/100 >= (min_purchase)) {
                //if gift is not present add it
                if (!$promo_variant) {
                  var promo1_data = {
                    quantity: 1,
                    id: promo_data.promo_spend_get_1_variant,
                    properties: {Info: "Gift product"},
                  };
                  theme.cart.addItemFromForm(promo1_data)
  ...
  theme.CartDrawer = (function() {
    ...
    CartDrawer.prototype = $.extend({}, CartDrawer.prototype, {
      ...
      buildCart: function(cart, openDrawer) {
        ...
        if (cart.item_count === 0) {
          ...
          data = {
            ...
            totalPrice: '<span class="hulkapps-cart-original-total">' + theme.Currency.formatMoney(cart.total_price, theme.settings.moneyFormat) + '<span>',
      ...
      updateItem: function(evt, key, qty) {
        ...
        var update_data = {'updates': {[key]:qty} };
        theme.cart.updateCartItem(update_data)
          .then(function(cart) {
            //is there an active campaign
            if(today >= promo1.from && today <= promo1.to){
              $promo_variant = cart.items.find(function (value) {
              ...
              if (cart.original_total_price/100 >= (min_purchase) ) {
                //if gift is not present and is not the one being removed add it
                if (!$promo_variant && promo1.key != promo_data.promo_spend_get_1_variant){
                  var promo1_data = {
                    quantity: 1,
                    id: promo_data.promo_spend_get_1_variant,
                    properties: {Info: "Gift product"},
                  ...
                  theme.cart.addItemFromForm(promo1_data)
                    .then(function(product) {
                      $(".product-single__form").find('.errors').remove();
  ...
  theme.AjaxProduct = (function() {
    ...
    function ProductForm($form) {
      ...
      if (this.$form.length) {
        ...
        $('.js-artisan-msg__chk').on('change', this.toggleArtisanMessage.bind(this));
        $('.js-artisan-msg__str').on("keyup",function() {
          var maxLength = $(this).attr("maxlength");
          if(maxLength == $(this).val().length) {
            $(".js-artisan-msg__lbl").show();
          } else {
            $(".js-artisan-msg__lbl").hide();
          }
        })
      }
    };
    ...
    ProductForm.prototype = $.extend({}, ProductForm.prototype, {
      addItemFromForm: function(evt, callback){
        ...
        var gift_data = {
          quantity: 1,
          id: this.$form.data('gift-variant'),
          properties: {
            'is_gift': true,
            'minimun': this.$form.data('gift-min'),
            'base_variant': this.$form.find('[name=id]').val(),
            'base_quantity': this.$form.find('[name=quantity]').val()
          }
        };
        ...
        var promo_data = {
          ...
          'promo_spend_get_1_from': "",
          'promo_spend_get_1_to': "",
          'promo_spend_get_1_price': "80",
        };

        var today = new Date();
        var promo1_from = new Date(promo_data.promo_spend_get_1_from);
        var promo1_to = new Date(promo_data.promo_spend_get_1_to);
        var min_purchase = parseInt(promo_data.promo_spend_get_1_amount);

        //TODO cambiar el orden de las campanias xq si tiene free gift a nivel de product no llega a la segunda
        theme.cart.addItemFromForm(data)
          .then(function(product) {
            //conditionally add gift products
            if (gift_data.properties.base_quantity >= gift_data.properties.minimun) {
              theme.cart.addItemFromForm(gift_data)
                .then(function(product) {
            ...
            // spend X and get Y
            else if(today >= promo1_from && today <= promo1_to) {
              var promo1_data = {
                  quantity: 1,
                  id: promo_data.promo_spend_get_1_variant,
                  properties: {Info: "Gift product"},
              ...
              theme.cart.getCart()
                .then(function(cart) {
                  ...
                  if (!$promo_variant && cart.original_total_price/100 >= (min_purchase) ) {
                    theme.cart.addItemFromForm(promo1_data)
                            .then(function(product) {
                              this.success(product);
                            }.bind(this))
      ...
      toggleArtisanMessage: function (evt) {
        $input = $(evt.target).siblings("input.js-artisan-msg__str");
  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        klarnaPlacement: 'klarna-placement',
        ...
        inventoryPolicyContainer: '[data-variant-inventory-policy]',
        currentInventoryPolicy: '#VariantInventoryPolicy-'  + sectionId,
  
        addToCart: '#AddToCart-' + sectionId,
        addToCartText: '#AddToCartText-' + sectionId,

        preorderTextCopy: '.pre-order',
        notifyWhenBackInStock: '#BIS_trigger',
        notifyWhenAvailable: '.BIS_trigger',
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      updateCartButton: function(evt) {
        ...
        var inventoryQuantity = parseInt($('.stock-qty').text(), 10);
        var inventoryPolicyContainer = $(this.selectors.inventoryPolicyContainer);
  
        if (variant) {
          if(inventoryQuantity <= 0) {
            // if template contains 'preorder' and current_variant.inventory_policy == 'continue'
            if(($(this.selectors.addToCart).closest('.product-section-preorder').length) && (inventoryPolicyContainer.data('variant-inventory-policy') == 'continue')) {
              // Available as Preorder, enable the submit button and change text
              $(this.selectors.addToCart).removeClass(classes.disabled).prop('disabled', false);
              $(this.selectors.addToCartText).html(theme.strings.preorder);
              $(this.selectors.preorderTextCopy).html(theme.strings.preorderTextCopy);
            } else { // regular product template
              ...
              $(this.selectors.preorderTextCopy).html('');
            }
            $(this.selectors.notifyWhenBackInStock).removeClass('hide').html(theme.strings.notifyWhenBackInStock).show();
          } else {
            // Available, enable the submit button and change text
            $(this.selectors.addToCart).removeClass(classes.disabled).prop('disabled', false);
            $(this.selectors.addToCartText).html(theme.strings.addToCart);
            $(this.selectors.notifyWhenBackInStock).hide();
            $(this.selectors.preorderTextCopy).html('');
          }
          $(this.selectors.notifyWhenAvailable).hide();
        } else {
          // The variant doesn't exist, disable submit button
          $(this.selectors.addToCart).addClass(classes.disabled).prop('disabled', true);
          $(this.selectors.addToCartText).html(theme.strings.unavailable);
          $(this.selectors.notifyWhenAvailable).html(theme.strings.notifyWhenAvailable).show();
          $(this.selectors.preorderTextCopy).html('');
        }
      },
      ...
      updatePrice: function(evt) {
        ...
        if (variant) {
          // Update Klarna Price
          // let splitBy = '&purchase_amount=';
          // let klarnaSrc = $(this.selectors.klarnaPlacement).find('iframe').attr('src').split(splitBy);
          // $(this.selectors.klarnaPlacement).find('iframe').attr('src', klarnaSrc[0] + splitBy + variant.price);
          ...
          if(typeof buddy_variant_price !== 'undefined'){
            ...
            // Regular price
            $(this.selectors.price).html(buddy_variant_price).show();
      ...
      updateInventory: function(evt) {
        ...
        var $inventoryPolicyContainer = $(this.selectors.inventoryPolicyContainer);
        var currentInventoryPolicy = $(this.selectors.currentInventoryPolicy + '-' + variant.id);

        // update variant.inventory_policy on variant change
        $inventoryPolicyContainer.data('variant-inventory-policy', currentInventoryPolicy.text().trim());
  
        // update variant.inventory_quantity on variant change
        if (variant && variant.inventory_management === 'shopify') {
          showInventory = true;
          quantity = window.inventories[this.sectionId][variant.id];

          $inventoryLabel.find('.stock-qty').html(quantity);
      ...
      colorSwatchHovering: function() {
        ...
        $swatches.on({
          mouseenter: function (evt) {
            ...
            var price = '<span class="money">' + $el.data('price') + '</span>';
            $('.grid-product__color-image--' + id).parent().find('.grid-product__color-image').removeClass('is-active');
            $('.grid-product__color-image--' + id)
              .css('background-image', 'url(' + image + ')')
              .addClass('is-active');
            $(".grid-product__link[data-p-id="+pid+"]").attr('href', $el.attr('href'));
            $("#QuickShopModal-"+pid).find(".variant-input[data-variant-id="+id+"] label").trigger('click');
            $(".grid-product__link[data-p-id="+pid+"]").find('.grid-product__price').html(price);
          }
        });
      },
